...
class BanditStrategy(object):	
    ...
    def __init__(self, bandits, choice_function):
        ...
        self.wins = np.zeros(n_bandits).astype(int)
        self.trials = np.zeros(n_bandits).astype(int)
        self.N = 0
        self.choices = []
        self.score = []
...
def epsilon_greedy(self, epsilon=0.1):
    ...
    if random.random() < epsilon:  # exploration
        ...
        # we add 1 in the denominator to avoid divide by 0
        return np.argmax(self.wins / (self.trials + 1))
...
def bayesian_bandit(self):
    ...
    Return the index of the winning bandit.
    '''
    return np.argmax(stats.beta.rvs(1 + self.wins, 1 + self.trials - self.wins))
